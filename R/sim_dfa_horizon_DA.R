...
sim_dfa_horizon_DA <- function (num_trends = 1, num_years = 20, num_ts = 4, num_horizons = 2, phi = c(0.8)
                        , loadings_matrix = matrix(nrow = num_ts, ncol = num_trends, rnorm(num_ts * num_trends, 0, 1))
                        , sigma = rlnorm(num_ts, meanlog = log(0.2), 0.1)
                        ...
                        , extreme_loc = NULL, nu_fixed = 100, user_supplied_deviations = NULL) 
{
  ...
  for (k in seq_len(d$sampling_args$data$K)) {
    ...
    if (is.null(extreme_value)) {
      ...
      if (k == 1) {
        x[1, extreme_loc] <- phi[k]*x[1, extreme_loc - 1] + 
          extreme_value
      }
      else {
        x[k, extreme_loc] <- phi[k]*x[k, extreme_loc - 1] + 
          devs[t]
      }
  ...
  sigmaH <- ar1_cor(num_horizons, rho = rho)
  for(i in 1:num_horizons){
    sigmaH[i,i] <- 1+0.125*sqrt(i-1)
  }
  
  SigmaHL <- sigma[1]*bdiag(replicate(d$sampling_args$data$P, sigmaH, simplify=FALSE))
  ...
  list(y_sim = y_sim, pred = pred, x = x, Z = Z, sigma = sigma, phi = phi, sigmaH = sigmaH, SigmaHL = SigmaHL)
}

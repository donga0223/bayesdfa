...
#' }
fit_dfa_DAt <- function(y = y,
                    num_trends = 1,
                    num_horizons = 2,
                    varIndx = NULL,
                    var_locIndx = NULL,
                    var_horizonIndx = NULL,
...
##  if ( data_shape[1] == "wide") { ## ncol(y) < nrow(y) &&
##    warning(
...
##  }
  ...
  if (data_shape[1] == "wide") {
    N <- ncol(y) # number of time steps (T)
    #P <- nrow(y) # number of time series * horizons (L*H)
    H <- num_horizons
    P <- nrow(y)/H
    if (nrow(y) < 3) {
      stop(
  ...
  if (is.null(var_locIndx)) {
    var_locIndx <- rep(1, P)
  }
  n_locVariances <- length(unique(var_locIndx))
  
  if (is.null(var_horizonIndx)) {
    var_horizonIndx <- rep(1, H)
  }
  n_horizonVariances <- length(unique(var_horizonIndx))
  

  # indices of positive values - Stan can't handle NAs
  if (data_shape[1] == "wide") {
    row_indx_pos <- matrix(rep(rep(seq_len(P), each=H), N), P*H, N)[!is.na(y)]
    col_indx_pos <- matrix(sort(rep(seq_len(N), P*H)), P*H, 
                           N)[!is.na(y)]
    hor_indx_pos <- matrix(rep(seq_len(H), N*P), P*H, N)[!is.na(y)]
    n_pos <- length(row_indx_pos)
    row_indx_na <- matrix(rep(rep(seq_len(P), each=H), N), P*H, N)[is.na(y)]
    col_indx_na <- matrix(sort(rep(seq_len(N), P*H)), P*H, 
                          N)[is.na(y)]
    hor_indx_na <- matrix(rep(seq_len(H), N*P), P*H, N)[is.na(y)]
  ...
  data_list <- list(
    ...
    nZero = nZero,
    varIndx = varIndx,
    var_locIndx = var_locIndx,
    var_horizonIndx = var_horizonIndx,
    nVariances = nVariances,
    n_locVariances = n_locVariances,
    n_horizonVariances = n_horizonVariances,
    row_indx_z = row_indx_z,
    col_indx_z = col_indx_z,
    row_indx_pos = row_indx_pos,
    col_indx_pos = col_indx_pos,
    hor_indx_pos = hor_indx_pos,
    n_pos = n_pos,
    row_indx_na = row_indx_na,
    col_indx_na = col_indx_na,
    hor_indx_na = hor_indx_na,
    n_na = n_na,
    nu_fixed = nu_fixed,
  ...
  if (is.null(par_list)) {
    ...
    # family
    if (family %in% c("gaussian", "lognormal")) pars <- c(pars, "sigma_l", "sigma_h")

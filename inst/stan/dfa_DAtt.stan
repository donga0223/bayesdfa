...
transformed parameters {
  ...
  matrix[P*H,N] Apred; //vector[P] pred[N];   *****horizon dimension include*****
  matrix[P,K] Z;
  matrix[P*H,K] AZ;
  //matrix[P,N] yall;
  ...
  matrix[P*H*long_format*est_cor, N*long_format*est_cor] temp_sums;// temporary for calculations for MVN model
  matrix[P*H*long_format*est_cor, N*long_format*est_cor] temp_counts;// temporary for calculations for MVN model
  vector[P*H*long_format*est_cor] cond_sigma_vec;// temporary for calculations for MVN model
  vector[P*H*long_format*est_cor] cond_mean_vec;// temporary for calculations for MVN model
...
model {
  ...
  if(est_cor == 0) {
    if(long_format==0) {
      if(obs_model == 1) {for(i in 1:P) target += normal_lpdf(yall[i] | Apred[i], sigma_vec[i]);} // gaussian
    } else {

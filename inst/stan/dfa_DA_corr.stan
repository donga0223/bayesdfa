functions {
  ...
  vector sequence(int start, int n){
    vector[n] seq;
    for(i in 1:n){
      seq[i] = start+(i-1);
    }
  ...
  matrix for_est_cor(matrix A, int H){
    int N = dims(A)[2];
    int PH = dims(A)[1];
    int P = PH/H;
...
parameters{
  ...
  cholesky_factor_corr[H] Lcorr; // matrix for correlated errros
}
...
transformed parameters{
  ...
  if(est_cor == 1) {
    ...
    Sigma = quad_form_diag(Omega, sigma_horizon);
    if(H == 1){
      pred_H = pred;
    }else{
      pred_H = for_est_cor(pred, H);
    }
  }
}
...
model{
  ...
  if(est_cor == 1) {
    for(i in 1:N*P){
      target += multi_normal_cholesky_lpdf(col(yall_H,i) | col(pred_H,i), diag_pre_multiply(sigma_horizon, Lcorr));
    } 
  }else{
...
generated quantities{
  ...
  if(est_cor == 1){
    for(i in 1:P*N) {
      log_lik[i] = multi_normal_cholesky_lpdf(col(yall_H,i) | col(pred_H,i), diag_pre_multiply(sigma_horizon, Lcorr));
    }
  }else{
    for(n in 1:N) {
      for(p in 1:P*H) {
